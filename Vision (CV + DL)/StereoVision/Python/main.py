...
cv_file.open('C:/Users/vardh/Documents/StereoVision/Python/stereoMap.xml', cv2.FileStorage_READ)
...
########################################################################################
    ...
    if ret_right==False or ret_left==False:                    
        ...
        tomatoes_right = shape.find_tomatoes(frame_right)
        tomatoes_left  = shape.find_tomatoes(frame_left)

        # Hough Transforms can be used aswell or some neural network to do object detection

        ################## CALCULATING TOMATO DEPTH #########################################################

        # If no tomato can be caught in one camera show text "TRACKING LOST"
        if len(tomatoes_right) == 0 or len(tomatoes_left) == 0:
            ...
            # Function to calculate depth of object. Outputs vector of all depths in case of several tomatoes.
            # All formulas used to find depth is in video presentation
            for i in range(min(len(tomatoes_right), len(tomatoes_left))):
                depth = tri.find_depth(tomatoes_right[i], tomatoes_left[i], frame_right, frame_left, B, f, alpha)
